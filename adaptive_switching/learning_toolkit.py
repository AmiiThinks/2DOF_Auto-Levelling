...
class TDLambdaLearner(Learner):
    ...
    def __init__(self, numTilings = 1, num_bins = 2, rlAlpha = 0.5, rlLambda = 0.9, rlGamma = 0.9, cTableSize=0):
        ...
        self.mem_size = 1048576 # 16,384 or 8,192 or 1,048,576 or 2,097,152 or 4,194,304 or 8,388,608 or 16,777,216 or 33,554,432
...
#         print 'normalized prediction = ' + str(self.normalized_prediction)
    ...
    def update(self, features, gamma, rho, target=None):
        if features != None:
            self.learn(features, gamma, rho, target)
            return self.prediction
        else: return None
      
    def learn(self, state, gamma, rho, reward):
...
#             print 'reward = ' + str(reward)
            ...
            #print 'deeeelta =', delta
            for i in self.traceH.getTraceIndices():
                self.theta[i] += delta * (self.rlAlpha / self.numTilings) * self.traceH.getTrace(i) * rho # delta * alpha/N * e
                #print 'theeeta =', self.theta[i]
...
#             print 'lastS' + str(self.lastS)
        self.lastQ = currentq
        self.lastS = state
       # print 'lastSSSS ', self.lastS
        self.prediction = currentq
       # print 'predictiooon =', self.prediction
        self.num_steps+=1
        self.verifier.updateReward(reward)
...
class GTDLambdaLearner(Learner):
    """
    Note: the TileCoder is Rich's Python version, which is still in Alpha.
    See more at: http://webdocs.cs.ualberta.ca/~sutton/tiles2.html#Python%20Versions
    ...
    """
    def __init__(self, numTilings = 1, num_bins = 2, rlAlpha = 0.5, rlLambda = 0.9, rlGamma = 0.9, rlBeta=0.0001, cTableSize=0):
        """ If you want to run an example of the code, simply just leave the parameters blank and it'll automatically set based on the parameters. """
        self.numTilings = numTilings
        self.num_bins = num_bins
        self.rlAlpha = rlAlpha
...
#         self.previous_state = [None for item in range(self.numTilings*(self.num_bins)**10)]
        ...
        self.traceH = TraceHolder(self.mem_size, 0.01, 1000) # TraceHolder(mem, minT, maxN)
        self.F = [0 for item in range(self.numTilings)] # the indices of the returned tiles will go in here
        self.F_old = [0 for item in range(self.numTilings)]
        self.theta = [0 for item in range(self.mem_size)] # weight vector
        self.h = [0 for item in range(self.mem_size)] # second weight vector
        self.x_t = [0 for item in range(self.mem_size)]
        self.x_tp1 = [0 for item in range(self.mem_size)]
        self.cTable = CollisionTable(cTableSize, 'super safe') # look into this...
        self.verifier = Verifier(self.rlGamma)
        self.eTh = 0.0
        self.hTx = 0.0
    ...
    def update_lambda(self, rllambda):
        self.rlLambda = rllambda
        return self.rlLambda

    def update(self, features, gamma, rllambda, rho, target=None):
        if features != None:
            self.learn(features, gamma, rllambda, rho, target)
            return self.prediction
        else: return None

    def learn(self, state, gamma, rllambda, rho, reward):
        """ Learning algorithm """

        """ Update variable gamma and lambda """
        self.update_gamma(gamma)
        self.update_lambda(rllambda)
        #print 'gamma', self.rlGamma
        #print 'rho', rho
        #print 'state', state
        #print 'lastS', self.lastS

        """ Determine active tiles, calculate current prediction values """
        self.loadFeatures(state, self.F)
        currentq = self.computeQ()  # computeQ returns w*x' (current prediction)
        # print 'traceindices =', self.traceH.getTraceIndices()

        """ Calculate the TD error """
        if self.lastS != None and self.lastH != None:
            delta = reward + self.rlGamma*currentq - self.lastQ # delta = r + gamma*w*x' - w*x
            #print 'delta =', delta

            """ If i in self.F, x_tp1=1 else x_tp1=0; likewise for F_old with x_t """
            ...
            """ Precompute eTh and hTx """
            self.eTh = 0.0
            self.hTx = 0.0
            # for i in range(0, self.mem_size):
            #     if self.traceH.getTrace(i) > 0:
            #         self.eTh += self.traceH.getTrace(i)*self.h[i]
            #     if self.x_t[i] > 0:
            #         self.hTx += self.h[i]*self.x_t[i]
            for i in self.F_old:
                self.hTx += self.h[i]  # The active indices in F_old are all equal to x_t=1.0, therefore hTx is the summation of h[i]*1.0
            for i in self.traceH.getTraceIndices():
                self.eTh += self.traceH.getTrace(i)*self.h[i]  # getTraceIndices returns all the nonzero trace indices. All the indices that equal 0 don't matter
            ...
            for i in self.traceH.getTraceIndices():
                if i in self.F:  # this avoids having to explicitly use the vector x_tp1
                    self.x_tp1[i] = 1.0
                else:
                    self.x_tp1[i] = 0.0
                self.theta[i] += delta * (self.rlAlpha / self.numTilings) * self.traceH.getTrace(i) - self.rlGamma * (1 - self.rlLambda) * (self.rlAlpha / self.numTilings) * self.traceH.getTrace(i) * self.x_tp1[i] * self.eTh
                if i in self.F_old:  # this avoids having to explicitly use the vector x_t
                    self.x_t[i] = 1.0
                else:
                    self.x_t[i] = 0.0
                self.h[i] += self.rlBeta * (delta * self.traceH.getTrace(i) - self.hTx * self.x_t[i])
                #print 'theta', self.theta[i]
                #print 'h', self.h[i]
            self.traceH.decayTraces(self.rlGamma*self.rlLambda)
            self.traceH.replaceTraces(self.F)
            self.traceH.decayTraces(rho)
        self.lastQ = currentq
        self.lastS = state
        self.lastH = self.h
        self.prediction = currentq
